...
def main(example, df, possible_subjects):
    ...
    sorted_idx = np.argsort(cos_tuts)[::-1] # sort by similarity
    cos_tuts = cos_tuts[sorted_idx]

    y = y[sorted_idx] # sort hourly rates by similarity
    sim_tuts.reset_index(drop=True, inplace=True)
    sim_tuts = sim_tuts.iloc[sorted_idx][cos_tuts>.6]
    ...
    # ax = plt.gca()
    # ax.plot(x, kde(x))
    # ax.fill_between(x, kde(x), alpha=.5)
    ...
    # plt.tight_layout()
    # plt.show()
    # plt.savefig('./app/static/img/tmp_fig.png', dpi=96)


    return sim_tuts, y, cos_tuts
