...
class clean():
    ...
    def subjects_to_boolean_features(self):
        ...
        possible_subjects = \
                        pickle.load(open('./data/possible_subjects.pkl','rb'))
        # possible_subjects = \
        #                 pickle.load(open('../data/possible_subjects.pkl','rb'))
    ...
    def get_subject_categories(self):
        ...
        subject_cats = pickle.load(open('./data/cat_features.pkl', 'rb'))
        # subject_cats = pickle.load(open('../data/cat_features.pkl', 'rb'))
        ...
        def bool_subject_cats(row, subject_cats):
            row_cats = row.keys()
            row_cats = rename_columns(row_cats)
            return_dict = {}
        ...
        self.cat_features = subject_cats
...
class clean_to_sklearn():
    ...
    def to_X_matrix(self):
        ...
        #                                                     np.float(x))
        # print self.clean.df['zip_code']
        self.clean.df['zip_code'] = np.int(self.clean.df['zip_code'])
    ...
    def to_y_array(self):
        y = np.empty(1)
        y[0] = np.float(self.clean.df['hourly_rate'])
...
def clean_tutor(tutor_df):
    # Make duplicate row because I don't know how to make single-row'd DataFrame such that .apply() works well
    tutor_df = pd.concat([tutor_df, tutor_df], axis=1).transpose()

    cleaned = clean(df=tutor_df)
    cleaned.all_features()
    cleaned.df = cleaned.df.iloc[0]
    hourly_rate = int(cleaned.df['hourly_rate'])
    return cleaned.df, hourly_rate
